...
class FileReader(FileReaderBase):
    ...
    def generate_id(cls, df, session_time_interval, do_label_encode=True, only_last_metric=True):
        ...
        def get_logical_day(timestamp):
            """
            计算逻辑日：以凌晨4点为分界线
            如果时间小于4点，属于前一天的逻辑日
            如果时间大于等于4点，属于当天的逻辑日
            返回格式：YYYY-MM-DD (逻辑日期)
            """
            if timestamp.hour < 4:
                # 如果在0-4点之间，属于前一天的逻辑日
                logical_date = (timestamp - timedelta(days=1)).date()
            else:
                logical_date = timestamp.date()
            return logical_date
        ...
        prev_logical_day = None
        time_interval = []
        
        timestamps = df['UTCTimeOffset'].tolist()
        users = df['UserId'].tolist()
        
        for idx, (user, timestamp) in enumerate(tqdm(zip(users, timestamps))):
            if idx == 0:
                prev_logical_day = get_logical_day(timestamp)
                time_interval.append(None)
                continue
                
            current_logical_day = get_logical_day(timestamp)
            time_diff = timestamp - timestamps[idx - 1]
            
            if start_user != user:
                # 不同用户
                start_id += 1
                start_user = user
                prev_logical_day = current_logical_day
            elif current_logical_day != prev_logical_day:
                # 同一用户，但跨越了逻辑日分界线（凌晨4点）
                start_id += 1
                prev_logical_day = current_logical_day
