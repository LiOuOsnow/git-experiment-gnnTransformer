...
def ignore_first(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This is the safest strategy to avoid data leakage. Although it reduces the training data,
    it ensures that the model does not access the current trajectory information when predicting
    ...
    # 显式将第一个点的 query_trajectory_id 设置为 None
    ...
    # 显式将第一个点的 last_checkin_epoch_time 设置为 None
    df.loc[df['pseudo_session_trajectory_rank'] == 1, 'last_checkin_epoch_time'] = None
    
    # 忽略用户的第一个点（UserRank==1）
    df.loc[df['UserRank'] == 1, 'SplitTag'] = 'ignore'
    
    # 关键：恢复忽略每条轨迹的第一个点（rank==1）
    # 这是为了避免数据泄露，因为第一个点没有合法的历史轨迹可以查询
