...
def preprocess_nyc(path: bytes, preprocessed_path: bytes, session_time_interval: int = 1440) -> pd.DataFrame:
    ...
    # 设置FileReader的root_path，以便保存label encoding文件
    FileReader.root_path = path
    
    # 重新生成轨迹ID（使用新的按逻辑日划分的策略）
    df = FileReader.generate_id(
        df,
...
def preprocess(cfg: Cfg):
    ...
    if not osp.exists(sample_file):
        if 'nyc' == dataset_name:
            ...
            preprocessed_data = preprocess_nyc(
                data_path, 
                preprocessed_path, 
                cfg.dataset_args.session_time_interval
            )
