...
class MDC-Hyper(nn.Module):
    def __init__(self, cfg):
        super(MDC-Hyper, self).__init__()
        ...
        self.cl_loss_weight = getattr(cfg.model_args, 'cl_loss_weight', 0.1)
        self.cl_temperature = getattr(cfg.model_args, 'cl_temperature', 0.1)
        
        # Projection head for contrastive learning
        self.ssl_head = nn.Sequential(
            nn.Linear(self.checkin_embed_size, self.checkin_embed_size),
            nn.ELU(),
            nn.Linear(self.checkin_embed_size, self.checkin_embed_size)
        )
    ...
    def encode(self, data, mode='train'):
        input_x = data['x']  # [?, 8]
    ...
    def calc_contrastive_loss(self, z1, z2):
        ...
        # Simpler implementation of InfoNCE with in-batch negatives
        # z1 and z2 are positive pairs.
        # We compute logits for z1[i] against all z2[j]
        logits = torch.matmul(z1, z2.T) / self.cl_temperature
        labels = torch.arange(batch_size).to(z1.device)
        loss = F.cross_entropy(logits, labels)
        
        # To be symmetric, we can also compute z2 against z1
        logits2 = torch.matmul(z2, z1.T) / self.cl_temperature
        loss2 = F.cross_entropy(logits2, labels)
        
        return (loss + loss2) / 2

    def forward(self, data, label=None, mode='train'):
        x = self.encode(data, mode=mode)
        logits = self.linear(x)
        loss = self.loss_func(logits, label.long())
        
        if mode == 'train' and self.cl_loss_weight > 0:
            # Generate second view by encoding again (different dropout)
            x2 = self.encode(data, mode=mode)
            
            # Project to SSL space
            z1 = self.ssl_head(x)
            z2 = self.ssl_head(x2)
            
            cl_loss = self.calc_contrastive_loss(z1, z2)
            loss = loss + self.cl_loss_weight * cl_loss
